...
function enter(pi) {
	...
	if (prop == null || prop == 0) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You have so many thoughts swirling through your mind that the mirror won't show you what you want. You should try again later or move to a different channel."));
		return false;
		}
	...
	if (prop == null || prop == 0) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You have so many thoughts swirling through your mind that the mirror won't show you what you want. You should try again later."));
		}
		return false;
}