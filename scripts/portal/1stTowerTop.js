...
function enter(pi) {
	...
	if (pi.getPlayer().itemQuantity(4032831)) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Return Jenn's brother to Jenn first."));
		return true;
		}
	...
	if (pi.getPlayer().itemQuantity(4032858)) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Save Jenn's brother."));
		return true;
		}
	...
	if (pi.getMap(211060201).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You can't get to the roof of the tower without a key."));
		return false;
}