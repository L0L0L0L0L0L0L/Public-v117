...
function enter(pi) {
	...
	if (pi.getMap(910050000).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "It looks like someone is receiving Bowman training inside. Enter later so you don't cause any disruptions."));
		return false;
		}
