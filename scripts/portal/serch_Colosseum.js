...
function enter(pi) {
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(31010)).getStatus() == 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("You found the mammoth! Go tell Michael about this."));
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You found the mammoth! Go tell Michael about this."));
		}
