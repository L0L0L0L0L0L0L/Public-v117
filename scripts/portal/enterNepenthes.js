...
function enter(pi) {
	...
	if (prop == null || prop == 0) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Cannot enter the Neglected Strolling Path. Try again a little later."));
		return false;
		}
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Cannot enter the premise without a purpose."));
		return false;
}