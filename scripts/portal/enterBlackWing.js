...
function enter(pi) {
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(22611)).getStatus() == 1) {
		var em = pi.getEventManager("q22611");
	...
	if (pi.getPlayer().hasEquipped(1003134)) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You can't enter without proof that you are a member of the Black Wings. Equip something with the Black Wings' logo on it to enter."));
		return false;
}