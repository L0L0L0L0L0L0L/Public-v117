...
function enter(pi) {
	if ((pi.getPlayer().getJob() == 2000 || pi.getPlayer().getJob() == 2100) && pi.getPlayer().getLevel() < 20) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Anyone who has no business entering the Mirror Cave will be denied access."));
		return false;
		}
