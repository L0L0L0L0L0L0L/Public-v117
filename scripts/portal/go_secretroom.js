...
function enter(pi) {
	if (pi.getPlayer().itemQuantity(4032405) < 1) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The room seems to be locked tight. You can acquire the Secret Key by rescuing Princess Violetta."));
		return false;
		}
		for (var i = 0; i < num; i++)
	if (pi.getMap(map + i).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You are using the Secret Key to enter."));
		return true;
		}
