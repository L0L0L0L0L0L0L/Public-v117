...
function enter(pi) {
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(20749)).getStatus() != 1) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The door is impossible to open."));
		return false;
		}
	if (pi.getMap(922000030).getCharacters().size() > 0) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The door has opened. Now entering the next dimension."));
