...
function enter(pi) {
	...
	if (pi.getMap(924000200).getCharacters().size() > 0) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "There is an another adventurer being tested inside... Move to a different channel so you don't disturb them."));
		return false;
		}
		pi.getMap(924000200).resetFully();
		pi.getPlayer().changeMap(pi.getMap(924000200), pi.getMap(924000200).getPortal(2)); //噴火龍的森林
		pi.getPlayer().startMapTimeLimitTask(600, pi.getMap(240020400));
		return true;
		}
