...
function enter(pi) {
	...
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(quest[i])).getStatus() == 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You can only enter the Penguin Training Center if you are getting trained by Puo."));
		return false;
}