...
function enter(pi) {
	...
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(22567)).getStatus() > 1) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "I don't want to enter again."));
		return false;
		}
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The entrance was destroyed. It will be difficult to get in."));
		return false;
}