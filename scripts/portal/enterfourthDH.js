...
function enter(pi) {
	...
	if (pi.getMap(913020300).getCharacters().size() > 0) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The 4th Hall is only available to those that are training for the Level 110 skill."));
		return false;
}