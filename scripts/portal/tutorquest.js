...
function enter(pi) {
	...
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(quest[i])).getStatus() == state[i]) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, pi.getPlayer().getMap().getId() == 130030001 ? "I should talk to Kisha first." : "You must complete the quest before proceeding to the next map."));
		return false;
}