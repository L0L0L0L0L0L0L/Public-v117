...
function enter(pi) {
	...
	if (y < 4) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("" + (parseInt(y) + 1) + "/5 Regions Completed"));
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("The One Who's Touched the Sky title in progress."));
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The One Who's Touched the Sky title in progress. " + (parseInt(y) + 1) + "/5 Regions Completed"));
		return false;
		}
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("Earned The One Who's Touched the Sky title!"));
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You have earned <The One Who's Touched the Sky> title. Please see NPC Dalair to receive your Medal."));
		}
		return false;
}