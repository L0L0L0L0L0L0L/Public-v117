...
function enter(pi) {
	if (pi.getPlayer().getJob() == 2001 && pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(22005)).getStatus() < 1) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "I have no business in the Lush Forest."));
		return false;
		}
	...
	if (pi.getPlayer().itemQuantity(4032449)) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The piglet has already been rescued. No need to go to the Lush Forest."));
		return false;
		}
