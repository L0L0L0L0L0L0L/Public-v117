...
function enter(pi) {
	if (pi.getPlayer().itemQuantity(4032179)) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The Ereve search begins."));
	...
	if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(quest[i])).getStatus() == 1) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Cannot pass through without an Investigation permit."));
		return false;
		}
		pi.getPlayer().changeMap(pi.getMap(130010000), pi.getMap(130010000).getPortal(3)); //修煉森林1
		return true;
