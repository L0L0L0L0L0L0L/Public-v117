...
function enter(pi) {
	if (pi.getPlayer().getJob() == 2000 && pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(21014)).getStatus() < 2) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "The town of Rien is to the right. Take the portal on the right and go into town to meet Lilin."));
		return false;
		}
