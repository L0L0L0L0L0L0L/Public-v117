...
function enter(pi) {
	...
	if (pi.getMap(910150230).getCharacters().size() > 0) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "This field is already in use. Try again later, or change channels and try again."));
		return false;
		}
