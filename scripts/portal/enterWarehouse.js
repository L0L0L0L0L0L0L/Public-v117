...
function enter(pi) {
	...
	if (pi.getMap(930010000).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "You've entered the library. Use your mouse to search for the safe."));
		pi.getPlayer().startMapTimeLimitTask(600, pi.getMap(300000010));
		return true;
		}
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "It looks like the path hasn't opened up yet. Try again a little later."));
		return false;
		}
