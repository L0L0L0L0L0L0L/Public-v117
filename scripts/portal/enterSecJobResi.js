...
function enter(pi) {
	...
	if (pi.getMap(931000420).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.getTopMsg("Thieves have attacked! Defeat all the thieves and then go see Ace the Pilot."));
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "Thieves have attacked! Defeat all the thieves and then go see Ace the Pilot."));
