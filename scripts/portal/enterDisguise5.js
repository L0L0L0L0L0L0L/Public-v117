...
function enter(pi) {
	...
	if (pi.getMap(913002400).getCharacters().size() > 0) {
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "This forest is already being searched by someone else. Come back later."));
		return false;
		}
		...
		pi.getPlayer().startMapTimeLimitTask(600, pi.getMap(130010200));
		return true;
		}
