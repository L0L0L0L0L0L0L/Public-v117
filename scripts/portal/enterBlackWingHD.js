...
function enter(pi) {
	...
	if (pi.getMap(map[i]).getCharacters().size() < 1) {
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "" + text[i]));
		...
		pi.getClient().getSession().write(Packages.tools.packet.CWvsContext.serverNotice(5, "There's a loud noise coming from in there... Maybe you shouldn't go in."));
		return false;
}